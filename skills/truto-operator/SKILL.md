...
- An **MCP tool** exposed by an MCP server errors, returns nothing, or returns unexpected output → [Debug an MCP tool call](./references/debug-mcp-tool-call.md).
...
| An **MCP tool call** errors, returns nothing, or returns unexpected output | [P8 · Debug an MCP tool call](./references/debug-mcp-tool-call.md) |
...
| [P8 · Debug an MCP tool call](./references/debug-mcp-tool-call.md) | MCP tools as thin wrappers over proxy/unified calls: not-exposed vs broken, token/scope, argument-schema mismatch, and when it's really the underlying call. |
