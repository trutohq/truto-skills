...
| `query_schema` / `body_schema` / `response_schema` (legacy inline) | JSON-schema-like declaration | **Prefer documentation rows** (`type`: `query_schema`, `body_schema`, `response_schema`) — Capabilities and MCP read from the `documentation` table. Inline config fields are legacy fallbacks only. |
...
The new integration now appears in your environment's Link UI. Generate a link token, open Link with `truto link-tokens create -b '{"tenant_id":"acme-test","is_sandbox":true}'` (the `tenant_id` auto-creates a [tenant](./core-resources.md#tenants) when the account connects, or pre-create one with `truto tenants create --id acme-test` if you want metadata attached first), paste your test API key, and the connection should land. Confirm with:
...
- Per-method documentation rows (`description`, `query_schema`, `body_schema`, `response_schema`) and `examples` — generated by `truto integrations build` docs phase or edited in the dashboard; consumed by MCP tool listings.
- `rate_limit`.
