...
The link token carries a `tenant_id`. If a [tenant](./core-resources.md#tenants) with that ID doesn't already exist in the environment, Truto auto-materialises one during step 1 (as long as the ID matches `[A-Za-z0-9._:@+\-]{1,256}`). You can pre-create the tenant with `POST /tenant` if you want `metadata` (tier, region, plan) attached before the customer connects.
