...
| **Core Resources**              | `integrations` (incl. `init`, `validate`), `accounts` (aliases: `integrated-accounts`), `tenants` (incl. `create-bulk`), `environments`, `environment-integrations` (aliases: `env-integrations`; incl. `override-auth`, `override-pagination`, `override-rate-limit`, `override-webhook`, `show-override`), `api-tokens` | Platform entity management                                                                           |
| **Unified Model Customization** | `unified-models`, `unified-model-mappings`, `env-unified-models`, `env-unified-model-mappings`, `unified-mappings` (`build`/`validate`/`apply`)                                                                                                                                                | Base + per-environment unified model definitions and field mappings; AI-build mapping rows with `unified-mappings build` (see the **truto-unified-mappings** skill) |
...
- **`unified-mappings` (AI build) vs `unified-model-mappings` (admin CRUD)** — confusingly close names, different tools. `truto unified-mappings build/validate/apply` is the **AI build flow** that generates JSONata mapping rows into a local `<integration>.<model>.mappings.json` and pushes them (see the **truto-unified-mappings** skill). `truto unified-model-mappings` is the **admin CRUD** command for those rows (`unified-model-resource-method`). Build with the former; hand-edit individual rows with the latter.
...
- **`override` is a full replace on API and CLI `update`** — `PATCH` / `update -b '{"override":...}'` replace the stored `override` column wholesale. Always send the complete object (existing keys + changes). A partial body wipes omitted slots and `resources` keys. Only the `override-*` helpers fetch+merge for you. For `resources`: `show-override` → merge locally → `update` with the full override. See [Customizing Integrations §5](../truto/references/customizing-integrations.md#5-override-resources-full-override-required).
