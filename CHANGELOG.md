...
- `skills/truto-unified-mappings/references/agent-and-tools.md` — The two build modes (default agentic loop with plan → route → sample → generate → commit → finalize, and the `--structured` deterministic per-cell pipeline), the full agent tool list grouped by purpose (corpus, proxy + routing, sampling, generation, validation, finalize, server web tools), the live → DB → source → scrape sampling ladder, read-only live sampling (security), `--no-web-search`, prompt-caching/cost behavior (`agent.usage` log events), and the JSONL debug log.
